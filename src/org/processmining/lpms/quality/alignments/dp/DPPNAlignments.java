...
public class DPPNAlignments implements PNAlignments {
    ...
    private PNMatchInstancesRepResult computeSingleTrace(AcceptingPetriNet apn, XTrace trace, int traceIndex) throws AStarException {
        ...
        // all transitions
        ArrayList<Transition> transitions = apn.getNet().getTransitions().stream()
                .filter(transition -> !transition.isInvisible()).collect(Collectors.toCollection(ArrayList::new));
        // transition to index
        ...
        for (Map.Entry<Transition, DependencyExpression> entry :
                DirectDependencyComputer.computeVisibleTransitionsOnly(apn).entrySet()) {
        ...
        int[][] dp = new int[transitions.size()][trace.size()];
        ...
        for (int i = 0; i < transitions.size(); i++) {
            DPAlignmentResult alignmentResult = null;
            if (eventActivities.get(0).equals(indexToTransition.get(i).getLabel()) &&
                ...
                alignmentResult = new DPAlignmentResult(1, false);
        ...
        for (int j = 1; j < trace.size(); j++) {
            ...
            for (int i = 0; i < transitions.size(); i++) {
                // if no matching labels and not an invisible transition
                if (!indexToTransition.get(i).getLabel().equals(eventActivities.get(j))) {
                    // the number of occurrences stays the same as until the previous event
                    dp[i][j] = dp[i][j - 1];
                    if (alignmentsPerTransitionLast.get(i) == null) {
                        alignmentsPerTransitionCurrent.add(null);
                    } else {
                    ...
                    Pair<Integer, DPAlignmentResult> resultPair = computeCountOfNewOccurrences(dp,
                            ...
                            transitionDependencyMap.get(i), j, indexToTransition, transitionToIndex,
                            alignmentsPerTransitionLast);
                    dp[i][j] = dp[i][j - 1] + resultPair.getFirst();
                    if (resultPair.getFirst() == 0) {
                        alignmentsPerTransitionCurrent.add(null);
                    } else {
    ...
    private Pair<Integer, DPAlignmentResult> computeCountOfNewOccurrences(int[][] dp,
        ...
        if (dependencyExpression.equals(SingleExecutionDependency.INSTANCE)) {
            return new Pair<>(1,
                    DPAlignmentResult.createAllLogMoves(column));
        }
        if (dependencyExpression instanceof ActivityDependency) {
            ...
            return new Pair<>(
                    dp[depIndex][column - 1], alignmentsPerTransitionLast.get(depIndex));

        }
        if (dependencyExpression instanceof OrDependency) {
            int sum = 0;
            DPAlignmentResult collectedAlignments = null;
            for (DependencyExpression disjunt : ((OrDependency) dependencyExpression).getChildren()) {
                ...
                if (collectedAlignments == null) {
                    collectedAlignments = resultPair.getSecond().copy();
                } else {
        ...
        if (dependencyExpression instanceof AndDependency) {
            ...
            double product = 1;
            DPAlignmentResult crossProd = DPAlignmentResult.createAllLogMoves(column);
            ...
            if (crossProd == null && parsedProduct != 0 || crossProd != null && crossProd.getAlignments().size() != parsedProduct) {
                System.out.println("crossProd.getAlignments().size() = " + crossProd.getAlignments().size());
                System.out.println("parsedProduct = " + parsedProduct);
                crossProd.getAlignments().forEach(alignment -> System.out.println(Arrays.toString(alignment)));
...
//    private Pair<Integer, DPAlignmentResult> computeCountOfNewOccurrences(int[][] dp,
//                                                                          Map<Integer, DependencyExpression>
//                                                                          transitionDependencyMap,
//                                                                          DependencyExpression dependencyExpression,
//                                                                          int column,
//                                                                          Map<Integer, Transition> indexToTransition,
//                                                                          Map<Transition, Integer> transitionToIndex,
//                                                                          List<DPAlignmentResult>
//                                                                          alignmentsPerTransitionLast) {
//        if (dependencyExpression instanceof ActivityDependency) {
//            int depIndex = transitionToIndex.get(((ActivityDependency) dependencyExpression).getTransition());
...
//
//            for (Map.Entry<Transition, Set<Transition>> entry : sharedDependencies.entrySet()) {
//                product /= Math.pow(dp[transitionToIndex.get(entry.getKey())][column - 1], entry.getValue().size()
//                - 1);
//            }
//            int parsedProduct = (int) Math.rint(product);
//            if (Math.abs(product - parsedProduct) > 1e-9) {
...
//            return new Pair<>(parsedProduct, crossProd);
//        }
//        throw new IllegalArgumentException("Unknown dependency expression type: " + dependencyExpression.getClass()
//        .getName());
//    }
