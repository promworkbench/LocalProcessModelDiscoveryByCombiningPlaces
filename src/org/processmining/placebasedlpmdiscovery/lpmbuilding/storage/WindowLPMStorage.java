 ...
 */
public interface WindowLPMStorage<T extends LPM> extends Iterator<T> { // QA: Should not be called like this, the
    // name should
    // explain the type of storage.

    static WindowLPMStorage<LocalProcessModel> lada() {
     ...
     */
    Collection<T> removeLPMsFor(Activity activity, int position);
     ...
     */
    boolean addLPMFor(Activity activity, int position, T lpm);
     ...
     */
    default boolean addLPMsFor(Activity activity, int position, Iterable<T> lpms) {
